...
<div id="sortDiv" style="display: none;">
	<c:forEach var="sort" items="${sorts }">
		...
		<input type="hidden" name="<%=com.lmiky.jdp.web.constants.Constants.HTTP_PARAM_SORT_ORDERBY_NAME %>" value="${sortPropertyName}"/>
		<input type="hidden" name="<%=com.lmiky.jdp.web.constants.Constants.HTTP_PARAM_SORT_TYPE_NAME_PREFIX %>${sortPropertyName}" value="${sort.sortType}" />
	</c:forEach>
</div>
